...
Network<DataType>::Network(NetworkType type, std::string_view name, const int af,const int soketType,const int protocol, const u_short port,
...
template <typename DataType>
void Network<DataType>::StartProcessingData()
{
    if (_type == server)
    {
...
template <typename DataType>
void Network<DataType>::SetProcessingDataFunc(NetworkDataProcessingFuncType func)
{
    if (NULL != func)
    {
...
template <typename DataType>
void Network<DataType>::SetNewClientFunc(NetworkNewClientConnectedFuncType func)
{
    if (NULL != func)
    {
...
template <typename DataType>
int Network<DataType>::Send(const char* data, const int lenght,const SOCKET sock)
{
...
template <typename DataType>
int Network<DataType>::Send(const char* data, const int lenght)
{
    int status = (-1);
    if (client == _type)
    {
...
template <typename DataType>
std::vector<SOCKET> Network<DataType>::GetAllConnectedClents()
{
    return _clientSockets;
}

template <typename DataType>  
NetworkType Network<DataType>::type()
{
    return _type;
}

template <typename DataType>
const std::string_view Network<DataType>::name()
{
    return _name;
}
...
template <typename DataType>
void Network<DataType>::acceptClients()
{
...
template <typename DataType>
void Network<DataType>::receiveClientData(const SOCKET sock)
{
    int id = getIdBySocket(sock);
    while(true)
    {        
...
template <typename DataType>
void Network<DataType>::clientSocketRealese(const SOCKET sock)
{
    int id = getIdBySocket(sock);
...
template <typename DataType>
int Network<DataType>::getIdBySocket(const SOCKET sock)
{
...
template <typename DataType>
DataType* Network<DataType>::DataToStrConverion(char *charStr)
{
    return (DataType*)charStr;
}

template <typename DataType>
char* Network<DataType>::StrToDataConverion(DataType* data)
{
    return (char*)data;
}

template class Network<char>;
template class Network<uint8_t>;
